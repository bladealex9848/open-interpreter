#!/usr/bin/env pwsh
# Script para actualizar el perfil de PowerShell para usar el wrapper directo
...
function Invoke-OpenInterpreter {
    ...
    # Ejecutar Open Interpreter a través del wrapper directo
    bash -c "/Volumes/NVMe1TB/GitHub/open-interpreter/oi_direct_wrapper.sh $($Arguments -join ' ')"
}
...
Write-Host "Se ha actualizado el perfil de PowerShell para usar el wrapper directo." -ForegroundColor Green
